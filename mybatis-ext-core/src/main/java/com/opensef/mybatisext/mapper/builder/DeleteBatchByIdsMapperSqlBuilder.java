...
public class DeleteBatchByIdsMapperSqlBuilder implements MapperSqlBuilder {
    ...
    private String makeDeleteSql(EntityInfo entityInfo) {
        return String.format("DELETE FROM %s WHERE %s IN <foreach collection='" + BaseMapperParamConstant.ID_LIST + "' item='id' open='(' separator=',' close=')'>%s</foreach>",
    ...
    private String makeLogicDeleteSql(EntityInfo entityInfo) {
        ...
        return String.format("<foreach collection='" + BaseMapperParamConstant.ID_LIST + "' item='id' separator=';'> UPDATE %s <set><trim suffixOverrides=','> %s </trim></set> WHERE %s </foreach>", entityInfo.getTableName(),
                String.join("\n", setConditions),
                condition(entityInfo.getColumnIdName(), "id")
        );
    }

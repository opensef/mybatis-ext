...
public class UpdateAllBatchMapperSqlBuilder implements MapperSqlBuilder {
    ...
    public MapperSql build(Class<?> entityClass, Object... args) {
        ...
        for (EntityInfo.FieldInfo fieldInfo : entityInfo.getFieldInfoList()) {
            if (!fieldInfo.getIdField()) {
                setConditions.add(condition(fieldInfo.getColumnName(), "entity." + fieldInfo.getFieldName(), fieldInfo.getTypeHandler()) + ",");
            }
        }
